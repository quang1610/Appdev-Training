...
class CatTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.CatName.text = Cat.catArr[indexPath.row].name
        cell.CatImage.image = Cat.catArr[indexPath.row].image
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(segue.identifier == "CatDetailSegue") {
            let destVC = segue.destination as! DetailViewController
            let indexPath = self.tableView.indexPath(for: sender as! CatTableViewCell)
            let row = indexPath?.row
            
            destVC.segueName = String(Cat.catArr[row!].name)
            destVC.segueImage = Cat.catArr[row!].image
            destVC.segueAge = String(Cat.catArr[row!].age)
            destVC.segueType = String(Cat.catArr[row!].type)
        }
    }
